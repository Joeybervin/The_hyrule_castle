...
function getGameDifficulty() {
    ...
    (0, display_lib_1.displayMenu)('difficulté', ['Normal', 'Difficile', 'Infernal', 'Retour']);
...
// ! A REFAIR : LE TYPAGE N'EST PAS BON
...
function getGameFloorQuantity() {
    ...
    console.log('Combien de d\'étage votre tour ?');
    console.log(floorNumberSelection.map((level, index) => `[${index + 1}] ${level}`).join('    '));
    let answer = readlineSync.question(' : ');
    const validAnswerRegExp = (0, utils_1.createNumberRangeRegExp)(1, 4);
    if (!validAnswerRegExp.test(answer)) {
        console.log('Erreur ! Entrer une valeur valide .');
