...
function isPlayerMeetRequirements(player, trapRoomRequiment) {
    ...
    if (player[trapRoomRequiment.stat] >= parseInt(trapRoomRequiment.requirement)) {
        (0, utils_1.logCenteredText)('Félicitations tu regroupe les conditions requises afin de ne subir aucun dommage', ' ');
        (0, utils_1.logCenteredText)('N\'abandonne pas aventurier ! Le village compte sur toi', ' ');
        (0, utils_1.logCenteredText)('récompense : +1 coins', ' ');
        player.coins += 1;
    }
    (0, utils_1.logCenteredText)('Malheureusement tu ne regroupes pas les conditions requises pour passer cette room sans dommages en la quittant', ' ');
    (0, utils_1.logCenteredText)(`En quittant cette room tu perdras ${getRandomDamage} hp`, ' ');
