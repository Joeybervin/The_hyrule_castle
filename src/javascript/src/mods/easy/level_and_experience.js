...
function getRandomExp(player, battleResult, gameStatesRandomExpActivate) {
    if (battleResult === 'winner' && gameStatesRandomExpActivate === true) {
        ...
        (0, utils_1.logCenteredText)(`Vous avez gagné \x1b[37;1;3m${exp} xp\x1b[0m`, ' ');
...
function playerLevelUp(player) {
    ...
    if (player.exp >= levelUpPoint) {
        ...
        while (player.exp >= levelUpPoint) {
            ...
            player.level += 1;
            (0, utils_1.logCenteredText)(`\x1b[4mVous passez au niveau supérieur\x1b[0m`, ' ');
            (0, utils_1.logCenteredText)(`\x1b[30;47;1m niveau ${player.level} \x1b[0m`, '*');
            (0, utils_1.logCenteredText)(`Bravo vous avez atteint le \x1b[37mniveau   ${player.level}   \x1b[0m`, ' ');
            (0, utils_1.logCenteredText)('- Vous avez obtenu une récompense \x1b[32;1m+1\x1b[0m', ' ');
...
function levelUpPlayerStats(player) {
    ...
    (0, utils_1.logCenteredText)(`Récompense : amélioration de vos \x1b[37m${statToUpgrade}\x1b[37m`, ' ');
    (0, utils_1.logCenteredText)(`Ancienne stat : \x1b[31;1m${player[statToUpgrade]}\x1b[0m ${statToUpgrade}   |   Nouvelle stat : \x1b[32;1m${player[statToUpgrade] + statIncreaseValue}\x1b[0m ${statToUpgrade}`, ' ');
